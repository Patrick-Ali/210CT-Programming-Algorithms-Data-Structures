  	...
  	std::vector<Vertex*> Dijkstra(int start)
  	{
  	    ...
  	    else
  	    {
      	    ...
      	    while(!pq.empty())
      	    {
      	        ...
      	        std::cout<<"Current: "<< current->data<< " Weight: " << current->tentWeight << std::endl;
...
int main() 
{
  ...
  std::vector<Vertex*> check = g1.Dijkstra(0);
  
  for(int i = 0; i < check.size(); i++)
  {
      Vertex* current = check[i];
      std::cout << current->data <<" "<< current->tentWeight <<std::endl;
  }
