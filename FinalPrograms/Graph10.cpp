...
class Edge
{
 ...
 public:
    ~Edge()
    {
        this->node1 = nullptr;
        this->node2 = nullptr;
        // int data1 = this->node1->data;
        //  int data2 = this->node2->data;
         
        std::cout << "I was once connected now I am no more" <<std::endl;
        
    //     Vertex* temp = this->node1;
        
    //     for(int i = 0; i < temp->adjacencyList.size(); i++)
  	 //   {
  	 //       Edge* hold = temp->adjacencyList[i];
  	 //       if(hold->node1->data == data1 and hold->node2->data == data2)
  	 //       {
  	 //         temp->adjacencyList[i] = nullptr;  
  	 //       }
  	 //      	else if(hold->node2->data == data1 and hold->node1->data == data2)
  	 //       {
  	 //         temp->adjacencyList[i] = nullptr;  
  	 //       }
